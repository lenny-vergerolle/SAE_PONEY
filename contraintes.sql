...
begin
    ...
    SELECT poids into poidsPers from ADHERENT NATURAL JOIN RESERVER WHERE idAdh = new.idAdh and idPoney = new.idPoney;
    SELECT poidsMax into poidsMaxPoney from PONEY NATURAL JOIN RESERVER WHERE idAdh = new.idAdh and idPoney = new.idPoney;

    if poids > poidsMax then
        set mes = concat('Impossible !! Ce poney ne supportera pas ce poids ! ', new.poids);
